...
export class NgxCronService {
  ...
  private validate(
    ...
    const err = this.validateCronExpression(
      cron,
      data.isQuartz,
      configOverrides
    );
    ...
    if (
      data.minuteInterval > MAX_MINUTES ||
      data.minuteInterval === 0 ||
      data.minute > MAX_MINUTES
    ) {
    ...
    if (
      (daysMax !== null && data.day > daysMax) ||
      data.day > MAX_DAYS_OF_MONTH ||
      data.day === 0
    ) {
  ...
  private secondOrMinutesErrorDescription(
    period: 'seconds' | 'minutes',
    cron: any
  ) {
  ...
  private getSeconds(expressionParts: string[]): number | null {
    const v = this.getSegment(expressionParts[0])[0][0];
    return typeof v === 'number' ? v : null;
  }

  private getSecondInterval(expressionParts: string[]): number | null {
    const v = this.getSegment(expressionParts[0])[0][1];
    return typeof v === 'number' ? v : null;
  }

  private getMin(expressionParts: string[]): number | null {
    const v = this.getSegment(expressionParts[1])[0][0];
    return typeof v === 'number' ? v : null;
  }

  private getMinInterval(expressionParts: string[]): number | null {
    const v = this.getSegment(expressionParts[1])[0][1];
    return typeof v === 'number' ? v : null;
  }

  private getHour(expressionParts: string[]): number | null {
    const v = this.getSegment(expressionParts[2])[0][0];
    return typeof v === 'number' ? v : null;
  }

  private getDay(expressionParts: string[]): number | null {
  ...
  private getTime(expressionParts: string[]): moment.Moment | null {
    ...
    return typeof h === 'number' && typeof m === 'number'
      ? moment.tz([1990, 1, 1, h, m, s || 0], this.timezone)
      : null;
  }

  private getMonth(expressionParts: string[]): keyof typeof Month | null {
    const v = this.getSegment(expressionParts[4])[0][0];
    return typeof v === 'number' ? (Month[v - 1] as keyof typeof Month) : null;
  }

  private getDaysMax(expressionParts: string[]): number | null {
    const v = this.getSegment(expressionParts[4])[0][0];
    return typeof v === 'number' ? new Date(2008, v, 0).getDate() : null;
  }

  private getDow(expressionParts: string[]): keyof typeof Weekday | null {
